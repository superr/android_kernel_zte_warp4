...
static int mdss_panel_parse_dt_compare_id(struct device_node *node,u32 *id)
{
	int rc;
	u32 lcd_product = 0;
	bool default_lcd_enabled = false;
	/*****************************************************/
		rc = of_property_read_u32(node, "zte,lcd-product", &lcd_product);
		if (rc)
		{
			printk("LCD  panel kernel ID not found ! \n");
			return -EINVAL;
		}
		
		printk("LCD  panel kernel ID = %d \n",lcd_product);
		return 0;// disable this funciton
	/*****************************************************/
	if(*id){
		rc = of_property_read_u32(node, "zte,lcd-product", &lcd_product);
		if (rc) {
		...
		if(lcd_product == *id)
			...
			default_lcd_enabled = of_property_read_bool(node,
	...
	bool cmd_cfg_cont_splash)
{
	...
	rc = mdss_panel_parse_dt_compare_id(node,&LcdPanleID);
	//if(rc){
